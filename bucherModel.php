...
class bucherModel
{
    ...
    public function __construct($show, $sortBy, $direction, $filter, $search, $searchBy)
    {
        ...
        if ($result->num_rows > 0){
            while($row = $result->fetch_assoc()){
                $temp = ['id'=>$row['id'], 'title'=>$row['kurztitle'], 'category'=>$row['kategorie'], 'sold'=>$row['verkauft'], 'buyer'=>$row['kaufername'], 'author'=>$row['autor'], 'content'=>$row['title'], 'condition'=>$row['zustand']];
